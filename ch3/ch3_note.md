...
- JavaScript 在執行錯誤時會拋出一個「例外」（Exception），而該例外內容通常是一個包含錯誤訊息的「錯誤物件」（Error Object）
- 「錯誤物件」，包含了錯誤的相關資訊，例如錯誤訊息、錯誤類型、堆疊追蹤等。
...
```js
let user = null;
console.log(user.name); // 這行程式碼會拋出 TypeError，因為不能讀取 null 的屬性
console.log('done'); // 這行程式碼不會被執行，因為前一行程式碼拋出了例外，導致程式中止執行
```

注意：在 JavaScript 中，`a / 0` 不會拋出例外，而是回傳 `Infinity` 或 `-Infinity`。
...
try {
    let user = null;
    result = user.name; // 這行程式碼會拋出 TypeError
} catch (error) { // 捕捉到例外後, 取得例外內的錯誤物件，並將它指派給 error 變數
...
導致高度循環複雜度的原因是重覆許多相同邏輯判斷與程式碼。可以看到 `total += items[i].price` 這行程式碼在多個條件分支中都出現了，這樣會讓程式碼變得冗長、難以閱讀和維護。
...
修改的策略: 
> 將複雜的程式碼拆分成多個簡單的函式，每個函式只負責一件事情，這樣可以降低每個函式的循環複雜度，提高程式碼的可讀性和可維護性。
...
抽取這些邏輯成一個獨立的函式來處理，這樣就可以降低主程式的循環複雜度。
...
// 定義一個函式來計算單一商品的價格，根據價格和會員狀態來計算折扣
// 將價格區間和會員狀態的判斷邏輯從主程式中抽取出來，讓主程式更簡潔
...
### 1.概念理解題（Conceptual Understanding）

#### Q1
什麼是「條件式（conditional statement）」？  
它在程式中的主要用途是什麼？



#### Q2
`if...else if...else` 與 `switch-case` 的主要差異是什麼？  
在什麼情境下，使用 `switch-case` 會更適合？



#### Q3

什麼是 switch-case 的 **fall-through** 行為？  
在什麼情況下使用它是合理的？



#### Q4
什麼是「循環複雜度（Cyclomatic Complexity）」？  
為什麼循環複雜度高的程式碼會較難閱讀與維護？



### 2. 技術緣由與設計決策題（Design Reasoning）

#### Q5

在以下兩種情況中，你會選擇哪一種？請說明理由：
...
### 4. 程式碼品質判斷（Code Quality Judgement）
