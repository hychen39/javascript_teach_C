...
- 理解物件的基本角色：物件是用屬性與方法描述現實世界實體的資料結構
- 掌握建立物件的主要方式：物件字面語法適合建立單一物件，class 適合建立多個具有相同結構的物件
- 理解 class 的核心組成：constructor、實例屬性、原型方法，以及使用 `new` 建立實例的流程
- 熟悉物件屬性的存取與操作：點運算子 `.`、方括號 `[]`、動態屬性、新增屬性與刪除屬性
- 認識封裝與成員可見性：公開成員、私有屬性、私有方法、getter、setter，以及 static members 的使用情境
- 建立 Clean Code 判斷能力：用存取語法表達意圖，避免 magic string，並用「告訴它，而不是詢問它」來設計物件行為
...
展現的語義：將一個函式表達式賦值給名為 drive_forward 的屬性鍵（Key）。
...
此屬性必須在類別定義時**在建構函式外宣告**，並用 `#` 前綴來修飾屬性名稱。
...
- 進而使得使用者在物件外部處理邏輯，違反了 Clean Code 原則: 「告訴它，而不是詢問它」原則 (Tell, Don't Ask)
  - 物件應該自己處理自己的狀態與行為，而**不是讓外部去詢問它的狀態然後在外部處理邏輯**。
...
如果一定要提供私有屬性的方法，則**應封裝對私有屬性設定時的「商業規則」或「驗證規則」**。
...
- 物件的基本觀念
  - 物件是現實世界實體的抽象化，可用屬性與方法描述一個實體的狀態與行為
  - 屬性是 key-value pair，方法是定義在物件上的函式，用來操作或表達物件行為
  - JavaScript 中大部分物件都是 `Object` 的實例，並可透過 prototype 取得共享方法

- 建立物件的方式
  - 物件字面語法 `{}` 適合快速建立單一物件
  - 物件字面語法中的方法可以使用 method shorthand，也可以用 function expression 作為屬性值
  - 當需要建立多個結構相同的物件時，class 比重複撰寫 object literal 更容易維護

- ES6 class 與實例
  - class 是建立物件的語法糖，底層仍然建立物件並透過 prototype 共享方法
  - `constructor` 用來初始化實例屬性
  - 實例屬性是每個實例自己擁有的資料，原型方法則由同一個 class 建立的實例共享
  - 使用 `new ClassName(...)` 可以建立 class 的實例

- 屬性存取與動態屬性
  - 固定且符合標準識別碼命名規範的屬性，優先使用點運算子 `.`
  - 屬性名稱來自變數、設定表、API 回傳值，或屬性名稱包含特殊字元時，使用方括號 `[]`
  - `[]` 內部的表達式會先被求值，再將結果作為屬性名稱
  - 使用方括號時應避免 magic string，必要時應以受控的變數或對照表管理動態鍵名

- 新增與刪除物件屬性
  - JavaScript 物件可以在執行時動態新增、修改與刪除屬性
  - 對某一個實例新增屬性，只會影響該實例，不會自動影響其他實例
  - `delete` 運算子可刪除物件自己的屬性，被刪除後再存取該屬性通常會得到 `undefined`

- 私有成員與封裝
  - 在 class 中使用 `#` 前綴可宣告私有屬性與私有方法
  - 私有成員只能在 class 內部存取，外部不能直接讀取或修改
  - 私有屬性可搭配公開方法、getter 或 setter 對外提供受控制的存取方式

- Getter 與 Setter
  - Getter 適合用於計算型欄位，或提供 read-only 的私有屬性
  - Setter 應用於需要驗證或封裝商業規則的設定流程
  - 不是每個私有屬性都需要 getter/setter；過度提供存取器會讓物件退化成單純的資料容器
  - 「告訴它，而不是詢問它」強調讓物件自己處理行為，而不是讓外部讀取狀態後再處理邏輯

- 靜態成員與 Symbol
  - Static members 屬於 class 本身，不屬於個別實例
  - 公開靜態成員可用 `ClassName.member` 存取，私有靜態成員可用 `static #member` 宣告並限制在 class 內部使用
  - Symbol 是唯一且不可變的 primitive value，可作為物件屬性名稱以避免命名衝突
  - 使用 Symbol 作為屬性名稱時，需要用計算屬性名稱與方括號語法存取

## 複習問題

1. 物件如何用屬性與方法描述現實世界的實體？請用車輛或使用者帳號作為例子說明。

2. 物件字面語法適合什麼情境？當需要建立多個相似物件時，為什麼 class 會比較適合？

3. `constructor`、實例屬性、原型方法各自負責什麼？它們在 class 中扮演什麼角色？

4. 使用 `new FIAT500(...)` 建立實例時，概念上會發生哪些事情？

5. 點運算子 `.` 與方括號 `[]` 在屬性存取上的差異是什麼？各自適合什麼情境？

6. 為什麼在 `[]` 中直接寫 `user['name']` 可能被視為 magic string？可以如何改善？

7. 對 `myFiat` 新增 `owner` 屬性，為什麼不會自動影響 `yourFiat`？這和實例的獨立性有什麼關係？

8. 私有屬性與私有方法如何用 `#` 宣告？它們如何幫助 class 封裝內部狀態與邏輯？

9. Getter 與 setter 各自適合什麼情境？為什麼不是每個私有屬性都應該提供 getter 和 setter？

10. 什麼是 static members？公開靜態成員、私有靜態成員和實例成員有什麼差異？
