...
- 如果 JS 沒有限制一定要宣告會發生什麼？
    - 拼字錯誤變成新變數
    - 沒有宣告的變數會成為全域變數，可能會污染**全域命名空間**，導致難以維護和除錯
...
- 若開發者沒有給予初始值，JS 會自動將變數初始化為 `undefined`，表示變數尚未賦值
...
Q: 什麼時候應該在宣告變數時同時賦值？什麼時候應該先宣告，之後再賦值？
- 若變數的初始值在宣告時就已經確定，建議在宣告時同時賦值，這樣可以提高程式碼的可讀性和維護性
  - 例如: 預設常數、初始狀態等
- 若變數的初始值需要根據程式執行過程中的條件或邏輯來決定，則可以先宣告，之後再賦值
  - 例如: 根據使用者輸入、API 回傳結果等動態資料來設定變數值
  - 將變數宣告與賦值分開，意圖上表達「變數的值會在程式執行過程中被設定」，而不是在宣告時就已經確定。

Example: 不同意圖的宣告與賦值方式

動態賦值的情境，先宣告後賦值： discount 的值會根據是否為 VIP 來決定，所以先宣告變數 discount，之後再根據條件賦值
...
宣告時同時賦值的情境: 設定預設值，表示 discount 的初始狀態為沒有折扣，之後再根據條件修改 discount 的值

若你要給 discount 一個預設值，則可以在宣告時同時賦值。
則上述程式碼可以改寫為:

```js
let discount = 0; // 預設值為 0
if (isVIP) {
    discount = 50; // 若是 VIP，則將 discount 設定為 50
}
```
...
- null 表示「刻意沒有值」, 開發者層面的「語意選擇」
  - null 的設計初衷：它被預期作為物件的佔位符（代表「這裡未來會放一個物件，但現在是空的」）。

語義上的區分有助於開發者表達意圖，並且在除錯時更容易理解變數的狀態。

因為語義上的不同，null 和 undefined 在運算中的行為也不同：

`undefine` 在數字運算中會被轉換為 NaN，表示「不是一個數字」:
...
arr2.push(4); // 新增元素 4 到 arr2 中

console.log(arr1); // ?
console.log(arr2); // ?
```

`arr1` 和 `arr2` 都指向同一個陣列資料: 
```
Stack                      Heap
-----                      -----
arr1  ───────────────►   [1,2,3]
...
因此，當 `arr2` 執行 `push(4)` 時，實際上是修改了堆積中的陣列資料，導致 `arr1` 和 `arr2` 都會反映出修改後的結果。
...
[Lab 02: 資料型別與型別轉換](labs/ch2_lab_02.md)
...
- **提升可讀性與可維護性**：  
  從變數宣告所在的作用域結構（全域 / 函式 / 區塊）與宣告方式（`let/const/var`），可以更清楚推測它在哪些地方「看得到、用得到」，讓程式結構更清楚。
...
`let` 及 `const` 皆具有 TDZ 行為。

開發實務建議:
> 建議在開發中優先使用 let 和 const 來宣告變數，以避免 var 的提升行為帶來的潛在問題。
