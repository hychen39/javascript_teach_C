...
一、應使用「明確型別轉換 (Explicit Conversion)」的情況

1. **商業邏輯與金額計算**  
   例如價格、折扣、稅額計算，必須確保型別正確，避免隱性轉換造成錯誤。

2. **資料來自外部來源時**  
   例如：HTML 表單、API 回傳資料、JSON 解析結果，通常為字串型別，應明確使用 `Number()`、`parseInt()` 等轉換。
...
👉 原則：**重要邏輯、商業計算、資料驗證 → 使用明確轉換。**


#### 二、可接受「自動型別轉換 (Type Coercion)」的情況
...
3. **學習或練習理解語言規則時**  
   用於觀察 JavaScript 轉換規則，但不建議作為正式商業邏輯。

👉 原則：**簡單輸出或可預測情境 → 可接受自動轉換。**



### 總結

- 自動型別轉換：方便，但可能降低可讀性與安全性。
- 明確型別轉換：較安全、可控，適合正式專案與商業系統。
