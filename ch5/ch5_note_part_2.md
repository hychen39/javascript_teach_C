...
- 在陣列中新增、更新與刪除元素，並理解哪些操作會直接改變原始資料
- 如何把陣列展開成一串值，或把多個值收集成一個陣列
- 排序與反轉的基本觀念，並知道數字排序需要額外指定規則
- 常見的陣列迭代方法，理解它們分別適合過濾、尋找、轉換、逐一處理與歸納資料
- 一般函數與 lambda 函數的差別，知道何時適合用具名函數，何時適合用簡短的匿名函數
- 了解在需要更細緻流程控制時，為什麼有時仍要自己寫迴圈或使用迭代器
...
rest parameter（剩餘參數）是「函式定義端」的語法：用 `...` 把呼叫時傳入的「值清單」收集成一個陣列，方便在函式內部處理。

你雖然看不到內建函式的實作，但很多 **Standard built-in objects** 的 API 在文件/簽名上會用 `...` 表示它們接受「不定個引數」（概念上就像內部把引數收集起來處理）。
...
小結：
- spread `...arr`：陣列 ➜ 值清單（用來呼叫吃不定引數的 API）
- rest `(...args)`：值清單 ➜ 陣列（用在「定義函式」時收集不定引數）
...
### Q: 為何有時仍要自行迭代？迭代方法的限制？

雖然迭代方法很方便，但它們並不能取代所有的迴圈。

有些情況下，我們仍然需要自己寫 `for` 或 `for...of` 來控制整個迭代流程。
...
例如 `forEach()` 就不能使用 `break` 提前結束：

```js
let scores = [55, 70, 82, 91, 40];

for (let score of scores) {
  if (score < 60) {
    console.log('發現不及格成績，停止檢查');
    break;
  }
}
```
...
console.log(max, maxIndex); // 9 3
```

**3. 不是每種需求都剛好對應一個迭代方法**

迭代方法有固定用途，但真實問題不一定剛好符合：
- 想找元素，可用 `find()`
- 想過濾元素，可用 `filter()`
- 想轉換元素，可用 `map()`

如果需求混合了很多步驟，硬要用迭代方法串接，程式可能反而變得不容易理解。

**4. 有時候效能或中間資料也是考量**

像 `filter().map()` 這樣的串接很清楚，但也可能建立中間陣列。

在資料量很大、或特別重視效能時，自己寫一次迴圈就完成所有處理，有時會更合適。
...
- 迭代方法適合常見、單一、清楚的資料處理工作
- 自行迭代適合需要流程控制、複雜邏輯、或一次完成多件事的情境
- 重點不是「哪個一定比較好」，而是選擇更適合當下問題的工具
