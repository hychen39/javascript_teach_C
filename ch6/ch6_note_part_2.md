...
![](img/nest_func_var_scope_rules.png)
...
詞法作用域分析原則：
- 函數可以存取哪些變數，是由函數「定義的位置」決定的。
- 查找變數時，會從目前作用域開始，逐層往外找。
- 可使用先前的 Rule 1~3 來分析巢狀函數的變數存取規則。

1. 在目前的程式碼中，回傳的函數物件可以存取變數, 由內到外的順序為:
- 回傳的函數內部的區域變數 c
- f 函數的區域變數(參數) b
- 全域變數 a


1. 當執行 `funY = f(20)` 時, `f(20)` 會回傳一個函數物件, 並把 `b` 的值設定為 20
  
2. 當執行 `const k = funY()` 時, `funY()` 會執行回傳的函數, 並計算 `a + b + c` 的值
   - `a` 的值是 10 (全域變數)
