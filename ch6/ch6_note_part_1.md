...
- 特性: 
  - 邏輯清晰, 適合用於定義程式碼結構中的主要功能塊。
  - 具函數提升(Function Hoisting)功能，允許「先呼叫再定義」的使用方式
...
JS 會自動將 `function add(a, b) { ... }` 提升到作用域的最上方，使得在宣告之前呼叫 `add(2, 3)` 仍然能正常執行。
...
意圖：先根據會員等級建立適合的「折扣函數」(操作邏輯)，之後只要把商品價格傳入該函數，就能計算折扣後價格。

![](img/return_function_object.png)
...
重要特性:
> 沒有自己的 `this`
> 繼承宣告它的作用域中的 `this` 值 (取決於「它在哪裡被定義」（定義時決定）)
> 細節在後續章節說明
...
希望在頁面一開啟就執行一些初始化邏輯，但又不希望初始化用的「暫時性變數或函數」變成全域變數去干擾到其他的程式碼。
...
執行後，IIFE 內的函數不會留在全域環境，無法被外部存取。
- 過了一段時間，記憶體回收機制（Garbage Collection）會自動回收 IIFE 內部的變數和函數，釋放記憶體資源。
...
當接受函數作為參數時，高階函數可以「使用剩餘參數接受被傳入的函數的所有引數」，並在高階函數內部呼叫該函數。
...
![](img/high_order_fun_example.png)

### Lab 

[lab_06_03 簡易加法與乘法計算器](labs/lab_06_03.md)
